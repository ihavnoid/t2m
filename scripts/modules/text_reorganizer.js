...
class TextReorganizer {
  ...
  detectDepth(line, prevLine, prevDepth, context = {}) {
    ...
    // If we haven't determined the space-per-level yet (e.g., 2 or 4), 
  ...
  _buildHierarchyHTML(lines, depths) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      lineText = lineText.replace(/^[\s\-\*\+\•\>]+\s?/, "").replace(/^\d+[\.\)\s]+\s?/, "");
      ...
      if (depth === -1) {
        ...
        const comment = lines[i].trim().replace(/^\/\/\s?|^note:\s?|^remark:\s?/i, "");
        ...
        if (lastLiIdx !== -1) {
            html = html.substring(0, lastLiIdx) + commentToken + html.substring(lastLiIdx);
        }
        continue;
      }
