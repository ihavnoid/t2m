...
class Settings {
    ...
    init() {
        document
            ...
            .addEventListener("change", () => this.enableAutoUpdate());

        let setting;
        try {
            setting = JSON.parse(
                sessionStorage.getItem(this.prefix + "documentTitle"),
            );
        } catch (exception) {
            // Ignored
        }
    ...
    createRwKey() {
        console.assert(
            this.xhr.readyState === 0 || this.xhr.readyState === 4,
            "Illegal XHR state",
        );
            ...
            if (this.xhr.status === 200) {
                ...
                if (this.rwkey) {
                    ...
                    sessionStorage.setItem(
                        this.prefix + "documentTitle",
                        JSON.stringify(this.rwkey),
                    );
                } else {
                    window.editorPane.setEditable(false);
                    sessionStorage.setItem(
                        this.prefix + "documentTitle",
                        JSON.stringify(this.rokey),
                    );
                }
                ...
                setTimeout(
                    () => this.createRwKey(),
                    window.globals.query_retry_period,
                );
            }
        };
        this.xhr.onerror = () => {
        ...
        this.xhr.onload = () => {
            if (this.xhr.status === 200) {
                ...
                if (t["contents"]) {
                    ...
                    if (this.rwkey) {
                        if (t["lockdelay"] > 0) {
                            window.editorPane.setEditable(
                                false,
                                "Somebody else is editing... please wait",
                            );
                            setTimeout(
                                () => this.updateFromServer(key),
                                Math.min(
                                    window.globals.lock_poll_period,
                                    t["lockdelay"],
                                ),
                            );
                        } else {
    ...
    enableAutoUpdate() {
        ...
        if (autoupdateEl.checked) {
            ...
            this.evtSource = new EventSource(
                window.__serverBase__ + "/p/pollpage.php?k=" + this.rokey,
            );
    ...
    setText() {
        ...
        if (
            ...
            window.editorPane.getProcessed() ===
