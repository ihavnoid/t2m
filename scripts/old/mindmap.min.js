    ...
    $(document).ready(function() {
        ...
        editorPane.on("keydown", function(a) {
            ...
            if (a.ctrlKey && 219 == a.which) {
                ...
                var nodes = findSelectedNodes(this)
                for(var i=0; i<nodes.length; i++) {
                    nodes[i].fixed = false 
                    window.d.updateTextForNode(nodes[i])
                }
            }
            ...
            // TODO : move nodes around
                ...
                if (g <= c) {
                    ...
                    editorPane.set(h.substring(0, e + 1) + f + h.substring(lastBreak, h.length));
                    let s = a.shiftKey ? Math.max(e+1, g - Math.min(2, nbr)) : g + Math.min(2, nbr);
                    let e = a.shiftKey ? c - nbr : c + nbr
                    editorPane.setPos(s, e);
                }
            } else if (13 == a.which) {
                ...
                editorPane.set(h.substring(0, g) + aa + h.substring(g2, h.length));
                editorPane.setPos(g + aa.length, g + aa.length);
            }
        });
...
//        });
        ...
        $("#newBtn, .newBtn").on("touchstart click", function(a) {
            a.stopPropagation();
            a.preventDefault();
            hasChanged && !1 == confirm("Create a new mind map without saving this one first?") ? a = void 0 : (editorPane.set(""), d.clear(), savingState = "unsaved", viewcode = admincode = "", hasChanged = !1, $(".saveMsg").hide(), $.cookie("current-mindmap", !1, {
                expires: -1
            }), a = !1);
            return a
        });
        ...
        $("#drawBtn").on("touchstart click", function(a) {
            ...
            render();
            var g;
            editorPane.get() == t ? ($("#holder").popover({
    ...
    var I = function(q, r) {
        ...
        function findTextLocation(a) {
            var e = -1
            var g = editorPane.get().split(/\n/)
            var c = d.nodes.indexOf(a);
            for (var f = 0; f <= c; f++) {
                if(!g[f].match(/^\s*-/)) {
        ...
        function updateTextForCoordinates(m) {
            var g = editorPane.get().split(/\n/)
            var c = d.nodes.indexOf(m);
            var x = ""
            var pos = 0
            
            var [sel_begin, sel_end] = editorPane.getPos();
            var charpos = 0
            for (var f = 0; f <= c; f++) {
            ...
            var newarr = " "
            if(m.fixed) {
                newarr = " [" + Math.floor(m.x) + " " + Math.floor(m.y) + "] "
            }
            ...
            editorPane.setPos(sel_begin, sel_end);
        }

        v.add(l);
        l.move($("#viewer-container").innerWidth() / 2, $("#viewer-container").innerWidth() / 2)
        ...
        return that = {
            ...
            runSimulation: function() {
                ...
                d.nodes.forEach((x) => {
                    x.parent_dim = 0;
                    if(x.data.parent) {
                        x.data.parent.parent_dim += x.w + x.h
                    }
                });
                var linkLength = (link, index) => {
                    let sn = link.source
                    let dn = link.target
                    let w = (sn.w + dn.w) / 2
                    let h = (sn.h + dn.h) / 2
                    return Math.max( Math.sqrt(w*w + h*h), (sn.parent_dim + dn.parent_dim)/16)
                }
                var chargeFunc = (node, index) => {
                    return -Math.max( Math.sqrt(node.w * node.w + node.h * node.h), node.parent_dim / 4)
                }
                ...
                if(f) {
                    ...
                    f = d3.layout.force().linkDistance(linkLength).charge(chargeFunc).friction(e.friction).theta(0.99).gravity(e.gravity).size([1e-6,1e-6]).nodes(d.nodes).links(d.links)
                    f.start()
                    f.on("tick", function() {
                        var a = f.alpha();
            ...
            execute: function(a, g) {
                f && f.stop();
                while(that.text2mindmap(a) == false) {}
            ...
            zoom: function(a) {
                if(a > 0) {
                    a = Math.max(0.5, l.getScale().x * 1.1)
                } else {
                    a = Math.max(0.5, l.getScale().x / 1.1)
                }
                that.settings({
...
*/
            ...
            setNodeLocks: function(a) {
                d.nodes.forEach(function(b) {
                    b.data.parent &&
                        (b.fixed = a, updateTextForCoordinates(b))
                });

                that.execute(editorPane.get(), false)
                f && !1 == a && f.resume()
            },
            updateTextForNode: updateTextForCoordinates,
            ...
            setStartPosition: function(a) {
                if (!1 == a.data.parent) return a.x = 0, a.y = 0, a.px = a.x, a.py = a.y, a;
                a.x = a.data.parent.x;
                a.y = a.data.parent.y;
            ...
            newGroup: function(a) {
                ...
                if(a.data.comment != "") { // && that.settings("scale") * a.data.fontSize > 16) {
                    ...
                    var width = Math.max(c.getWidth(), cc.getWidth())
                    var height = c.getHeight() + cc.getHeight()
                    var rec = that.newRect(a, width, height);
                ...
                b.on("touchstart mousedown", function(b) {
                    ...
                    a.fixed = true;
                    C = n(b.pageX || b.originalEvent.targetTouches[0].pageX, b.pageY || b.originalEvent.targetTouches[0].pageY);
                });
                b.on("mouseover", function() {
                    B.style.cursor = "pointer";
                    var ee = findTextLocation(a)
                    var e = ee[0]
                    var c = ee[1]
                    if(-1 != e && "undefined" != typeof e) {
                        var gg = e;
                        editorPane.setPos(gg, e);
                        editorPane.scrollToLine(c);
                    }
                });
                b.on("mouseout", function() {
                    (m.length > 0) || (B.style.cursor = "move")
                });
            ...
            text2mindmap: function(a) {
                // BROKEN : this filters out unwanted text.  Unfortunately
                // the rest of the code assumes one line == one node, and there
                // are some places that are broken. :(
                var b = a.split(/\n/)
                var c = [], j = 0
                var comments = []
                ...
                try {
                    ...
                    var addedNodes = 0, removedNodes = 0 
