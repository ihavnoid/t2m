        ...
        $("#textArea").keydown(function(a) {
            ...
            // TODO : move nodes around
                ...
                if (g <= c) {
                    ...
                    this.selectionStart = a.shiftKey ? Math.max(e+1, g - Math.min(2, nbr)) : g + Math.min(2, nbr);
                ...
                $(this).val(h.substring(0,
                    g) + aa + h.substring(g2, h.length));
                this.selectionStart = g + aa.length;
                this.selectionEnd = g + aa.length
            }
        });
...
//        });
    ...
    var I = function(q, r) {
        ...
        function updateTextForCoordinates(m) {
            ...
            var sel_begin = t.get(0).selectionStart
            var sel_end = t.get(0).selectionEnd
            var charpos = 0
            for (var f = 0; f <= c; f++) {
            ...
            t.get(0).selectionStart = sel_begin
            t.get(0).selectionEnd = sel_end
        }
        ...
        return that = {
            ...
            runSimulation: function() {
                ...
                if(f) {
                    ...
                    f = d3.layout.force().linkDistance(linkLength).charge(chargeFunc).friction(e.friction).theta(0.99).gravity(0.01).size([1e-6,1e-6]).nodes(d.nodes).links(d.links)
                    f.start()
                    f.on("tick", function() {
                        var a = f.alpha();
...
*/
            ...
            setNodeLocks: function(a) {
                ...
                that.execute($("#textArea").val(), false)
                f && !1 == a && f.resume()
            },
            ...
            setStartPosition: function(a) {
                ...
                var c = p[a.data.parent.id]++,
                    b = 135 + 360 / b * c,
                    c = Math.max(30, 600 / Math.exp(a.data.level*0.7));
                a.x += Math.round(c * Math.cos(b / 360 * 2 * Math.PI) + Math.random());
                a.y += Math.round(c * Math.sin(b / 360 * 2 * Math.PI) + Math.random());
                a.px = a.x;
                a.py = a.y;
                return a
            },
