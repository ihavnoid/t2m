        ...
        return that = {
            ...
            runSimulation: function() {
                ...
                for(let i = 0; i < d.nodes.length; i++) {
                    let x = d.nodes[i];
                    x.parent_dim = x.w + x.h;
                }
                for(let i = d.nodes.length-1; i >= 0; i--) {
                    let x = d.nodes[i];
                    if(x.data.parent) {
                        x.data.parent.parent_dim += x.parent_dim/4;
                    }
                }
                if(d.nodes.length > 0) {
                ...
                //} else {
                    ...
                    f
                        .force("collide", d3.forceCollide((n) => {
                            return {x: n.w/2.2, y:n.h/2.2};
                        }).iterations(1).strength(0.2))
                        .force("link", d3.forceLink(d.links).distance((link) => {
                            return Math.max(link.source.w, link.source.h)/2
                                + Math.max(link.target.w, link.target.h)/2 + 10
                        }))
                        .force("center", d3.forceCenter(0, 0).strength(0.005))
                        .force("manybody", d3.forceManyBody().strength(-400).distanceMax(300))
                        // .force("x", d3.forceX())
                        // .force("y", d3.forceY())
                    f.on("tick", function() {
                        var a = f.alpha();
