        ...
        return that = {
            ...
            createSimulation: function() {
                ...
                f = d3.forceSimulation(d.nodes)
                    ...
                    }).iterations(3).strength(0.5))
                    .force("link", d3.forceLink(d.links).strength(0.1).distance((link) => {
                        return Math.max(20, 10+Math.sqrt(link.source.area)/2 - Math.min(link.source.w, link.source.h)/2);
                    }))
                    .force("center", d3.forceCenter(0, 0).strength(0.005))
                    .force("manybody", d3.forceManyBody().strength(-100).distanceMax(300))
                    // .force("x", d3.forceX())
                    // .force("y", d3.forceY())
