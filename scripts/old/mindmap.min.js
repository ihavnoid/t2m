        ...
        return that = {
            ...
            createSimulation: function() {
                ...
                for(let i = 0; i < d.nodes.length; i++) {
                    let x = d.nodes[i];
                    x.parent_dim = (x.w + x.h)/4;
                }
                for(let i = d.nodes.length-1; i >= 0; i--) {
                    let x = d.nodes[i];
                    if(x.data.parent) {
                        x.data.parent.parent_dim += x.parent_dim/2;
                    }
                }
                if(d.nodes.length > 0) {
                ...
                f = d3.forceSimulation(d.nodes)
                    ...
                    }).iterations(2).strength(0.1))
                    .force("link", d3.forceLink(d.links).distance((link) => {
                        return Math.max(20, link.source.parent_dim/4, link.target.parent_dim/4);
                    }))
