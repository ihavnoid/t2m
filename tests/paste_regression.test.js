...
describe("EditorPane - Paste and Link Regression Tests", () => {
    beforeEach(() => {
        ...
        document.body.innerHTML =
            '<div id="textedit"></div><div id="textedit_message"></div>';
        editorPane.init();
    ...
    function mockSelection(container, offset) {
                ...
                if (container.nodeType === 3) {
                    ...
                    if (node.nodeType === 11) {
                        while (node.firstChild)
                            parent.insertBefore(node.firstChild, next);
                    } else {
                    ...
                    if (node.nodeType === 11) {
                        while (node.firstChild)
                            container.insertBefore(node.firstChild, ref);
                    } else {
        ...
        vi.spyOn(editorPane, "getWindow").mockReturnValue({
    ...
    describe("Single-Node vs Multi-Node Paste", () => {
        it("should merge single-node HTML paste inline", async () => {
            editorPane.el.innerHTML =
                '<ul><li><span class="header">Parent</span></li></ul>';
        ...
        it("should move block-paste to the end of node and insert as siblings", async () => {
            editorPane.el.innerHTML =
                '<ul><li id="p1"><span class="header">Parent</span><br><span class="comment">Comment</span></li></ul>';
    ...
    describe("Comment Mode Paste", () => {
        it("should treat multi-line plaintext as comments when in a comment section", async () => {
            editorPane.el.innerHTML =
                '<ul><li><span class="header">Node</span><br><span class="comment"></span></li></ul>';
        ...
        it("should override comment mode when pasting structured HTML", async () => {
            editorPane.el.innerHTML =
                '<ul><li><span class="header">Node</span><br></li></ul>';
            const li = editorPane.el.querySelector("li");
            ...
            const rootLis = editorPane.el.querySelectorAll(
                "#textedit > ul > li",
            );
    ...
    describe("Link Support", () => {
        it("cleanupHTML should preserve <a> tags with href and target", () => {
            editorPane.el.innerHTML =
                '<ul><li><a href="https://google.com">Search</a></li></ul>';
            editorPane.cleanupHTML();
        ...
        it("updateTextForCoordinates should preserve <a> tags", () => {
            ...
            editorPane.updateTextForCoordinates([
                { nodenum: 0, frozen: true, xp: 50, yp: 50 },
            ]);
    ...
    describe("Comment Precision", () => {
        it("should preserve prefixes in structural comments (depth -2)", async () => {
            ...
            const html =
                '<ul><li>Node<br><span class="comment">// Important Code Comment</span></li></ul>';
            await editorPane._processPasteContent(html, "");
            expect(editorPane.el.innerHTML).toContain(
