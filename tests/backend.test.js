...
describe('Backend Core API', () => {
    ...
    it('p/w.php should write content', () => {
        ...
        const output = execSync(`${PHP_CMD} p/w.php "${title}" "${contents}" "${rwkey}"`).toString();
    ...
    describe('Garbage Collection (p/gc.php)', () => {
        ...
        it('should delete mindmaps older than gc_mindmap_ttl_days', () => {
            ...
            // Verify it is gone
            const checkRes = spawnSync('php', ['-r', `
                include "p/common.php";
        ...
        it('should delete orphaned images older than gc_image_ttl_days', () => {
            ...
            // Cleanup dummy files
            if (fs.existsSync(`images/${recentHash}.png`)) fs.unlinkSync(`images/${recentHash}.png`);
            if (fs.existsSync(`images/${referencedHash}.png`)) fs.unlinkSync(`images/${referencedHash}.png`);
        });
    });
});
